...
import { createLiveSource, type EventSourceLike, type LiveSourceStatus } from './live-source.js';
import { PROTOCOL_VERSION, SAMPLE_EVENT, createBatch, formatSseEvent } from './protocol.js';
...
function setup() {
  ...
  const sendBatch = (...samples: [string, Sample][]) => {
    stream.emit(
      SAMPLE_EVENT,
      JSON.stringify(createBatch(samples.map(([semanticKey, sample]) => ({ semanticKey, sample })), NOW)),
    );
  };
