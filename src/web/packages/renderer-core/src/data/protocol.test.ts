...
describe('decodeBatch', () => {
  ...
  it('preserves a non-ok status carrying no value (§83)', () => {
    ...
    const result = decodeBatch(
      JSON.stringify(createBatch([{ semanticKey: 'cpu.fan', sample: unavailable }], NOW)),
    );

    expect(result.ok).toBe(true);
    // The absence of `value` is the payload. A transport that helpfully
    // defaulted it to 0 would turn a gap into a reading.
    expect(result.ok && result.batch.samples[0]?.sample).not.toHaveProperty('value');
  });

  it('refuses a newer version on the version alone (§141)', () => {
    const result = decodeBatch(
      JSON.stringify({ version: PROTOCOL_VERSION + 1, sentAt: 'x', samples: [] }),
    );

    expect(result).toMatchObject({ ok: false });
    expect(result.ok === false && result.reason).toContain(String(PROTOCOL_VERSION + 1));
  });

  it('refuses an older version too', () => {
    expect(decodeBatch(JSON.stringify({ version: 0, sentAt: 'x', samples: [] })).ok).toBe(false);
  });
  ...
  it('refuses a frame missing sentAt or samples', () => {
    expect(decodeBatch(JSON.stringify({ version: PROTOCOL_VERSION })).ok).toBe(false);
    expect(decodeBatch(JSON.stringify({ version: PROTOCOL_VERSION, sentAt: 'x' })).ok).toBe(false);
  });
});

describe('formatSseEvent', () => {
  it('names the event and ends on a blank line', () => {
    expect(formatSseEvent(SAMPLE_EVENT, '{"a":1}')).toBe('event: samples\ndata: {"a":1}\n\n');
  });
