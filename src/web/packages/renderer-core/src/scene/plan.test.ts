...
function ok(value: number, unit = '%', sensorId = 'cpu.load.total'): Sample {
  return { sensorId, timestamp: new Date(NOW).toISOString(), status: 'ok', value, unit };
}
...
function documentWith(nodes: readonly ThemeNode[], globals?: ThemeDocument['globals']): ThemeDocument {
  return {
    schemaVersion: 1,
    id: 'demo',
...
describe('artboard', () => {
  it('carries size and fit mode through', () => {
    ...
    expect(result.artboard).toMatchObject({ width: 800, height: 480, fitMode: 'contain' });
  });
  ...
  it('resolves the background through globals', () => {
    const document = documentWith([], { palette: { bg: { name: 'Background', value: '#101216' } } });
      ...
      documentWith([
        {
          ...
          children: [{ id: 'r', type: 'rectangle', transform: { x: 10, y: 5, width: 20, height: 20 } }],
        },
      ]),
    );
...
describe('style resolution (§75)', () => {
  ...
  it('substitutes a global reference for its literal', () => {
    const result = plan(
      documentWith([{ id: 'r', type: 'rectangle', style: { fill: { ref: 'palette.accent' } } }], globals),
    );
  ...
  it('passes a local literal through', () => {
    const result = plan(
      documentWith([{ id: 'r', type: 'rectangle', style: { fill: { value: '#ff0000' } } }]),
    );
  ...
  it('reports an unresolvable reference instead of guessing a default', () => {
    ...
    // nothing to point at.
    const result = plan(
      documentWith([{ id: 'r', type: 'rectangle', style: { fill: { ref: 'palette.nope' } } }]),
    );
  ...
  it('keeps a literal falsy value rather than dropping it', () => {
    const result = plan(
      documentWith([{ id: 'r', type: 'rectangle', style: { opacity: { value: 0 } } }]),
    );
...
describe('text (§89)', () => {
  const node = (bindings: unknown[], runs: unknown[]): ThemeNode =>
    ({ id: 't', type: 'text', bindings, content: { runs } }) as unknown as ThemeNode;

  function segments(source: SampleStore | typeof emptySampleSource, bindings: unknown[], runs: unknown[]) {
  ...
  it('does not throw on a run whose binding is missing', () => {
    ...
    const result = segments(emptySampleSource, [], [{ kind: 'value', bindingId: 'ghost' }]);
  ...
  it('renders a text and a boolean sample', () => {
    ...
    expect(result.segments.map((s) => s.text)).toEqual(['RTX 4080', ' / ', 'on']);
  });
  ...
  it('flags an ok sample that carries no value at all as an error', () => {
    ...
    expect(result.segments[0]).toMatchObject({ text: MISSING_VALUE_TEXT, status: 'error' });
  });
...
describe('charts', () => {
  const chartNode = (family: string, settings: unknown, bindings: unknown[]): ThemeNode =>
    ({ id: 'c', type: 'chart', bindings, content: { family, settings } }) as unknown as ThemeNode;

  it('feeds the latest sample to a gauge', () => {
    const result = plan(
      documentWith([chartNode('gauge', defaultGaugeSettings, [{ id: 'b', semanticKey: 'cpu.load.total' }])]),
  ...
  it('gives a line chart its windowed history', () => {
    ...
    store.ingest(
      [
        ['cpu.load.total', { ...ok(10), timestamp: new Date(NOW - 3000).toISOString() }],
        ['cpu.load.total', { ...ok(20), timestamp: new Date(NOW - 2000).toISOString() }],
        ['cpu.load.total', { ...ok(30), timestamp: new Date(NOW - 1000).toISOString() }],
      ],
      NOW,
    );

    const result = plan(
      documentWith([chartNode('line', defaultLineSettings, [{ id: 'b', semanticKey: 'cpu.load.total' }])]),
      { source: store },
    );

    const content = result.nodes[0]!.content;
    if (content.kind === 'chart' && content.family === 'line') {
      expect(content.option.series[0]!.data.map((point) => point[1])).toEqual([10, 20, 30]);
    }
  });
  ...
  it('gives a bar chart one category per binding, in order', () => {
    ...
    if (content.kind === 'chart' && content.family === 'bar') {
      expect(content.option.series[0].data.map((d) => d.value)).toEqual([30, 70]);
    }
  });
  ...
  it('still emits a drawable option when a key is unmapped', () => {
    ...
    const result = plan(
      documentWith([chartNode('gauge', defaultGaugeSettings, [{ id: 'b', semanticKey: 'nope' }])]),
    );
...
describe('assets', () => {
  it('resolves an image through the caller-supplied resolver', () => {
    const result = plan(
      documentWith([{ id: 'i', type: 'image', content: { assetId: 'logo', fit: 'cover' } }]),
      { resolveAsset: (id) => (id === 'logo' ? 'blob:logo' : undefined) },
    );

    expect(result.nodes[0]!.content).toEqual({ kind: 'image', src: 'blob:logo', fit: 'cover' });
  });
  ...
  it('reports an asset it cannot resolve', () => {
    ...
    expect(result.issues[0]).toMatchObject({ code: 'unresolved-asset', nodeId: 'i' });
  });
});
