...
import { isGenericFamily, parseFontStack, requestedFontFamilies } from './fonts.js';
...
function planOf(nodes: readonly ThemeNode[]): ReturnType<typeof buildScenePlan> {
...
describe('parseFontStack', () => {
  it('splits a stack and strips quotes', () => {
    expect(parseFontStack("'Segoe UI Variable Display', \"Segoe UI\", system-ui, sans-serif")).toEqual([
...
describe('requestedFontFamilies', () => {
    ...
    const plan = planOf([
      {
        ...
        content: {
          runs: [
            { kind: 'literal', text: 'a', style: { fontFamily: { value: 'Iosevka' } } },
            { kind: 'literal', text: 'b' },
          ],
        },
      },
  ...
  it('de-duplicates, preserving first-seen order', () => {
    const plan = planOf([
      { id: 'a', type: 'text', style: { fontFamily: { value: 'B' } }, content: { runs: [] } },
      { id: 'b', type: 'text', style: { fontFamily: { value: 'A' } }, content: { runs: [] } },
      { id: 'c', type: 'text', style: { fontFamily: { value: 'B' } }, content: { runs: [] } },
    ]);
  ...
  it('returns nothing when no font is named', () => {
    expect(requestedFontFamilies(planOf([{ id: 'r', type: 'rectangle' }]))).toEqual([]);
  });
});
