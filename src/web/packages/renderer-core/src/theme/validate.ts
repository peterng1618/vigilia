...
export function validateThemeDocument(input: unknown): ValidationResult {
  ...
  if (
    metadata !== undefined &&
    issues.object(metadata, "/metadata", "metadata")
  ) {
    ...
    if (
      ...
      issues.add(
        "wrong-type",
        "/metadata/version",
        "version must be a semantic major.minor.patch string.",
      );
    }
  }
  ...
  validateBackgroundMedia(
    issues,
    resolveOptional(input["artboard"], "backgroundMedia"),
    input["assets"],
  );

  validateStyleValue(
    issues,
    resolveOptional(input["artboard"], "background"),
    "/artboard/background",
    globalKeys,
  );
  validateStyleValue(
    issues,
    resolveOptional(input["artboard"], "barColor"),
    "/artboard/barColor",
    globalKeys,
  );
  ...
  for (const [groupName, group] of Object.entries(value)) {
    ...
    if (!GLOBAL_GROUPS.includes(groupName as (typeof GLOBAL_GROUPS)[number])) {
      issues.add(
        "invalid-enum",
        groupPath,
        `Unknown global group. Expected one of: ${GLOBAL_GROUPS.join(", ")}.`,
      );
      continue;
    }
    ...
    for (const [entryId, entry] of Object.entries(group)) {
      ...
      if (typeof entry["name"] !== "string" || entry["name"].length === 0) {
        issues.add(
          "missing-field",
          `${entryPath}/name`,
          "A global entry needs a non-empty display name.",
        );
      }
...
function validateBackgroundMedia(
  issues: Issues,
  value: unknown,
  assets: unknown,
): void {
  ...
  for (const key of Object.keys(value))
    if (key !== "assetId" && key !== "fit")
      issues.add(
        ...
        `backgroundMedia has no "${key}" property.`,
      );
  issues.enumValue(
    value["fit"],
    ["contain", "cover"] as const,
    "/artboard/backgroundMedia/fit",
    "fit",
  );
  const asset = Array.isArray(assets)
    ? assets.find(
        (entry) => isRecord(entry) && entry["id"] === value["assetId"],
      )
    : undefined;
  if (
    !isRecord(asset) ||
    !["image", "svg", "video"].includes(String(asset["kind"]))
  ) {
    issues.add(
      "unresolved-asset-ref",
      "/artboard/backgroundMedia/assetId",
      "backgroundMedia must reference a declared image, SVG or video asset.",
    );
  }
}

function validateTypePreset(
  issues: Issues,
  value: unknown,
  path: string,
): void {
  ...
  if (
    ...
    issues.add(
      "missing-field",
      `${path}/family`,
      "A type preset needs a font family.",
    );
  }
  if (
    !issues.finiteNumber(value["size"], `${path}/size`, "size") ||
    (value["size"] as number) <= 0
  ) {
    issues.add(
      "out-of-range",
      `${path}/size`,
      "A type preset size must be above 0.",
    );
  }
  ...
  if (
    ...
    issues.add(
      "wrong-type",
      `${path}/weight`,
      "A type preset weight must be a string or finite number.",
    );
  }
  for (const key of ["letterSpacing", "lineHeight"] as const) {
    const raw = value[key];
    if (raw !== undefined && !issues.finiteNumber(raw, `${path}/${key}`, key))
      continue;
    if (key === "lineHeight" && raw !== undefined && (raw as number) <= 0) {
      issues.add(
        "out-of-range",
        `${path}/${key}`,
        "A type preset lineHeight must be above 0.",
      );
    }
  }
  ...
  if (face !== undefined) {
    ...
    issues.unknownKeys(
      ...
      `${path}/face`,
      "fontFaceReference",
      "A font face reference",
    );
    issues.stableId(
      face["assetId"],
      `${path}/face/assetId`,
      "A font face asset id",
    );
  }
  if (value["trioRole"] !== undefined) {
    issues.enumValue(
      value["trioRole"],
      ["heading", "body", "mono"] as const,
      `${path}/trioRole`,
      "A trio role",
    );
  }
    ...
    if (issues.stableId(asset["id"], `${path}/id`, "An asset id")) {
      ...
      if (ids.has(id)) {
        issues.add(
          "duplicate-id",
          `${path}/id`,
          `Asset id "${id}" is used more than once.`,
        );
      }
      ids.add(id);
    }
...
function validateAssetPath(issues: Issues, value: unknown, path: string): void {
  if (typeof value !== "string") {
    issues.add(
      "missing-field",
      path,
      "An asset needs a package-relative path.",
    );
    return;
  }
  ...
  if (!ASSET_PATH_PATTERN.test(value)) {
    issues.add(
      "invalid-asset-path",
      path,
      "An asset path must be under assets/ and use safe characters.",
    );
    return;
  }
...
function validateNodes(
  ...
  validateNodeList(issues, value, "/nodes", 0, {
    nodeIds,
    bindingIds,
    globalKeys,
    assetIds,
...
function validateNode(
  ...
  if (context.counter.nodes === MAX_NODE_COUNT + 1) {
    issues.add(
      "too-many-nodes",
      path,
      `The document exceeds the maximum of ${MAX_NODE_COUNT} nodes.`,
    );
  }
  ...
  if (
    !issues.enumValue(value["type"], NODE_TYPES, `${path}/type`, "A node type")
  ) {
    return;
  }
  ...
  const ownBindingIds = validateBindings(
    issues,
    value["bindings"],
    `${path}/bindings`,
    type,
...
function validateFontAsset(
  ...
  if (
    ...
    asset["family"].trim().length === 0
  ) {
    issues.add(
      "missing-field",
      `${path}/family`,
      "A font asset needs a font family.",
    );
  }
  if (
    typeof asset["weight"] !== "string" &&
    (typeof asset["weight"] !== "number" || !Number.isFinite(asset["weight"]))
  ) {
    issues.add(
      "wrong-type",
      `${path}/weight`,
      "A font asset weight must be a string or finite number.",
    );
  }
  issues.enumValue(
    asset["style"],
  ...
  if (asset["format"] !== "woff2")
    issues.add(
      "invalid-enum",
      `${path}/format`,
      "A font asset format must be woff2.",
    );
  if (typeof asset["path"] !== "string" || !asset["path"].endsWith(".woff2")) {
    issues.add(
      "invalid-asset-path",
      `${path}/path`,
      "A WOFF2 font asset path must end in .woff2.",
    );
  }
  if (
    ...
    issues.add(
      "wrong-type",
      `${path}/sourceUrl`,
      "A font asset needs an HTTPS Fontsource source URL.",
    );
  }
  ...
  if (
    ...
    !isHttpUrl(license["url"])
  ) {
    issues.add(
      "missing-field",
      `${path}/license`,
      "A font asset needs licence name and URL metadata.",
    );
  }
}
...
function validateStyleValue(
  ...
  if (typeof ref !== "string") {
    issues.add(
      "wrong-type",
      `${path}/ref`,
      "A global reference must be a string.",
    );
    return;
  }
  ...
  if (
    !GLOBAL_GROUPS.includes(group as (typeof GLOBAL_GROUPS)[number]) ||
    entryId.length === 0
  ) {
    issues.add(
      "unresolved-global-ref",
  ...
  if (!globalKeys.has(ref)) {
    issues.add(
      "unresolved-global-ref",
      `${path}/ref`,
      `Global "${ref}" is not defined in this document.`,
    );
  }
}
    ...
    if (issues.stableId(binding["id"], `${bindingPath}/id`, "A binding id")) {
      ...
      if (context.bindingIds.has(id)) {
        issues.add(
          "duplicate-id",
          `${bindingPath}/id`,
          `Binding id "${id}" is used more than once.`,
        );
      }
...
function validateContent(
  ...
  switch (type) {
    case "group": {
      ...
      if (!Array.isArray(children)) {
        issues.add(
          "missing-field",
          `${path}/children`,
          "A group must have a children array.",
        );
        return;
      }
      validateNodeList(
    ...
    case "video": {
      if (
        !issues.object(content, `${path}/content`, `A ${type} node's content`)
      ) {
        return;
      }
    ...
    case "rectangle": {
      ...
      if (
        !issues.object(content, `${path}/content`, "A rectangle node's content")
      ) {
        return;
      }
...
function validateTextContent(
  ...
  if (!Array.isArray(runs)) {
    issues.add(
      "missing-field",
      `${path}/runs`,
      "A text node needs a runs array (§89 styled runs).",
    );
    return;
  }
...
function validateTypePresetReference(
  issues: Issues,
  value: unknown,
  path: string,
  globalKeys: Set<string>,
  ...
  if (
    typeof value !== "string" ||
    !/^typePresets\.[A-Za-z0-9_-]{1,64}$/.test(value)
  ) {
    issues.add(
      "unresolved-global-ref",
      path,
      "A text run typePreset must reference typePresets.<id>.",
    );
  } else if (!globalKeys.has(value)) {
    issues.add(
      "unresolved-global-ref",
      path,
      `Type preset "${value}" is not defined in this document.`,
    );
  }
}
