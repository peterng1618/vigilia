  ...
  nodes: [
    {
      ...
      type: 'group',
      children: [
        { id: 'rect-1', type: 'rectangle', transform: { x: 10, y: 20, width: 30, height: 40 } },
...
describe('serializeThemeDocument', () => {
  ...
  it('does not depend on the key order of the input object', () => {
    ...
    expect(serializeThemeDocument(shuffled)).toBe(serializeThemeDocument(minimal));
  });
  ...
  it('drops a key explicitly set to undefined rather than emitting it', () => {
    ...
    // never involved.
    const withUndefined = { ...minimal, metadata: undefined } as unknown as ThemeDocument;
    expect(serializeThemeDocument(withUndefined)).toBe(serializeThemeDocument(minimal));
  });

  it('keeps a falsy value', () => {
    const json = serializeThemeDocument({
      ...minimal,
      nodes: [{ id: 'r', type: 'rectangle', visible: false, transform: { x: 0 } }],
    });
...
describe('round trip', () => {
  it('re-validates after a save and load', () => {
    const reloaded = validateThemeDocument(JSON.parse(serializeThemeDocument(minimal)));
