  ...
  return [
    {
      ...
      children: [
        ...
        },
        {
          ...
          bindings: [
            { id: "card-value", semanticKey: "cpu.load", precision: 0 },
          ],
          content: {
            runs: [
              {
...
describe("fresh ids", () => {
  ...
  it("rewrites a text run to the binding it now names", () => {
    ...
    const result = instantiate({ idPrefix: "w1", widgetGlobals });
    const text = [...walkNodes(result.nodes)].find(
      (entry) => entry.node.type === "text",
    )?.node;
  ...
  it("never renames a semantic key", () => {
    ...
    expect(
      [...walkBindings(result.nodes)].map((entry) => entry.binding.semanticKey),
    ).toEqual(["cpu.load", "cpu.load"]);
  });
...
describe("globals (§77)", () => {
  ...
  it("remaps a reference inside a styled run", () => {
    ...
    if (text?.type === "text") {
      expect(text.content.runs[0]?.style?.["color"]).toEqual({
        value: "#ffffff",
      });
    }
  });
...
describe("placement and provenance (§138)", () => {
  it("offsets the roots only", () => {
    ...
    const child = [...walkNodes(result.nodes)].find(
      (entry) => entry.depth === 1,
    )?.node;
  ...
  it("omits provenance when none is supplied", () => {
    expect(
      instantiate({ idPrefix: "w1", widgetGlobals }).nodes[0]!.provenance,
    ).toBeUndefined();
  });
});
...
describe("copy semantics", () => {
  it("shares no mutable structure with the source", () => {
    ...
    if (sourceGroup.type === "group" && copyGroup.type === "group") {
      ...
      expect(copyGroup.children[0]!.style).not.toBe(
        sourceGroup.children[0]!.style,
      );
    }
  });
  ...
  it("preserves chart settings exactly", () => {
    const result = instantiate({ idPrefix: "w1", widgetGlobals });
    const chart = [...walkNodes(result.nodes)].find(
      (entry) => entry.node.type === "chart",
    )?.node;
