...
import { reassignChartPaintReferences, resolveChartPaint } from './chart-paint.js';
...
describe('chart paint', () => {
  ...
  it('reassigns token references in chart settings before a palette deletion', () => {
    const settings = {
      ...
      progress: { kind: 'thresholds' as const, bands: [{ offset: 1, ref: 'palette.old' }] },
    };
    expect(reassignChartPaintReferences(settings, 'palette.old', 'palette.new')).toEqual({
      track: { ref: 'palette.new' },
      progress: { kind: 'thresholds', bands: [{ offset: 1, ref: 'palette.new' }] },
    });
  });
});
