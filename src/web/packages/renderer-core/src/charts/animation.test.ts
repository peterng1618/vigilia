...
describe('the default is a continuous glide', () => {
  ...
  it('lets the first appearance be curved', () => {
    ...
    expect(defaultAnimationSettings.appearMs).toBeLessThan(defaultAnimationSettings.durationMs);
  });
...
describe('toEngineAnimation', () => {
  ...
  it('falls back to the default when no settings are authored', () => {
    expect(toEngineAnimation(undefined, true).animationDurationUpdate).toBe(1000);
  });
  ...
  it('caps a duration that would put the display seconds behind the sensor', () => {
    const settings: AnimationSettings = { ...defaultAnimationSettings, durationMs: 60_000 };
    expect(toEngineAnimation(settings, true).animationDurationUpdate).toBe(5000);
  });
  ...
  it('replaces a nonsensical duration with the default rather than emitting it', () => {
    for (const durationMs of [-100, Number.NaN, Number.POSITIVE_INFINITY]) {
      const settings: AnimationSettings = { ...defaultAnimationSettings, durationMs };
      expect(toEngineAnimation(settings, true).animationDurationUpdate).toBe(1000);
    }
  });
});
...
describe('every family emits the animation fields', () => {
  ...
  it('line', () => {
    const option = buildLineOption(defaultLineSettings, [{ sensorId: 'a', samples: [] }], 0);
  ...
  it('bar', () => {
    const option = buildBarOption(defaultBarSettings, [{ sensorId: 'a', sample: sample() }]);

    expect(option.animationDurationUpdate).toBe(1000);
  });

  it('pie', () => {
    const option = buildPieOption(defaultPieSettings, [{ sensorId: 'a', sample: sample() }]);

    expect(option.animationDurationUpdate).toBe(1000);
  });

  it('all four go completely still when animation is off', () => {
    expect(buildGaugeOption(defaultGaugeSettings, sample(), false).series[0].animation).toBe(false);
    expect(buildLineOption(defaultLineSettings, [], 0, false).animationDurationUpdate).toBe(0);
    expect(buildBarOption(defaultBarSettings, [], false).animationDurationUpdate).toBe(0);
    expect(buildPieOption(defaultPieSettings, [], false).animationDurationUpdate).toBe(0);
  });
});
