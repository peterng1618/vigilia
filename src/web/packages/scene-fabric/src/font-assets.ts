...
export interface FontAssetLoadOptions {
  ...
  readonly createFontFace?: (family: string, source: ArrayBuffer, descriptors: FontFaceDescriptors) => LoadedFontFace;
}

/** Registers packaged faces before Fabric measures text and returns their isolated release handle. */
export async function loadFontAssets(options: FontAssetLoadOptions): Promise<() => void> {
  ...
  for (const asset of options.assets) {
    ...
    const face = asset as AssetReference & { readonly family?: unknown; readonly weight?: unknown; readonly style?: unknown };
    const bytes = options.bytes[asset.path];
    if (bytes === undefined || typeof face.family !== 'string' || (typeof face.weight !== 'string' && typeof face.weight !== 'number') || (face.style !== 'normal' && face.style !== 'italic')) {
      options.onError(`Could not load packaged font "${asset.id}": declared bytes or metadata are missing`);
      continue;
    }
    try {
      ...
      options.onError(`Could not load packaged font "${asset.id}": ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  return () => { for (const font of loaded) fonts.delete(font); };
}
