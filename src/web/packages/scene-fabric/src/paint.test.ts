...
describe('a box', () => {
  ...
  it('has no stroke width when it has no stroke', () => {
    ...
    expect(paintFor({ strokeWidth: 4 }, 'box')).toMatchObject({ stroke: null, strokeWidth: 0 });
    expect(paintFor({ strokeColor: '#f00' }, 'box')).toMatchObject({ strokeWidth: 0 });
  });

  it('paints a stroke only when it has both a colour and a width', () => {
    expect(paintFor({ strokeColor: '#f00', strokeWidth: 2 }, 'box')).toMatchObject({
      stroke: '#f00',
      strokeWidth: 2,
    });
  });
  ...
  it('turns an authored dash into a pattern scaled by the stroke', () => {
    const dashed = paintFor({ strokeColor: '#f00', strokeWidth: 2, strokeDash: 'dashed' }, 'box');
    const dotted = paintFor({ strokeColor: '#f00', strokeWidth: 2, strokeDash: 'dotted' }, 'box');
    ...
    // frame set.
    expect(paintFor({ strokeColor: '#f00', strokeWidth: 2 }, 'box')['strokeDashArray']).toBeNull();
  });

  it('builds a real Shadow, and clears it when there is none', () => {
    const shadow = paintFor(
      { shadowColor: '#000', shadowBlur: 8, shadowOffsetX: 2, shadowOffsetY: 3 },
      'box',
    )['shadow'];

    expect(shadow).toBeInstanceOf(Shadow);
    expect(shadow).toMatchObject({ color: '#000', blur: 8, offsetX: 2, offsetY: 3 });
    expect(paintFor({}, 'box')['shadow']).toBeNull();
  });

  it('refuses a negative blur rather than passing it on', () => {
    expect(paintFor({ shadowColor: '#000', shadowBlur: -4 }, 'box')['shadow']).toMatchObject({
      blur: 0,
    });
  });
});
...
describe('text', () => {
  it('takes its colour from `color` first, then `fill`', () => {
    ...
    expect(paintFor({ fill: '#111', color: '#222' }, 'text')).toMatchObject({ fill: '#222' });
  });
  ...
  it('paints an outline behind the glyph, as CSS did', () => {
    ...
    expect(paintFor({ strokeColor: '#000', strokeWidth: 1 }, 'text')).toMatchObject({
      stroke: '#000',
      paintFirst: 'stroke',
    });
  ...
  it('drops letter spacing rather than guessing without a font size', () => {
    ...
    expect(paintFor({ letterSpacing: 2 }, 'text')).not.toHaveProperty('charSpacing');
    expect(unsupportedPaint({ letterSpacing: 2 }, 'text')).toContain('letterSpacing');
  });
...
describe('what canvas cannot do', () => {
  it('reports tabular numerals, which have no canvas equivalent', () => {
    ...
    expect(unsupportedPaint({ tabularNumerals: true }, 'text')).toContain('tabularNumerals');
    expect(unsupportedPaint({ tabularNumerals: false }, 'text')).toEqual([]);
  });

  it('reports a dashed text stroke, which CSS could not do either', () => {
    expect(unsupportedPaint({ strokeDash: 'dashed' }, 'text')).toContain('strokeDash');
