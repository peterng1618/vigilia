...
describe('placementFor', () => {
  ...
  it('leaves the centre alone when the node is scaled', () => {
    ...
    const placement = placementFor(box({ x: 10, y: 20, width: 100, height: 50, scaleX: 2, scaleY: 3 }));

    expect(placement).toMatchObject({ left: 60, top: 45, scaleX: 2, scaleY: 3 });
  });

  it('carries rotation across as degrees, under Fabric’s name for it', () => {
    expect(placementFor(box({ width: 10, height: 10, rotation: 37 })).angle).toBe(37);
  });
  ...
  it('accepts a zero-sized box rather than inventing one', () => {
    ...
    // rectangle on screen.
    expect(placementFor(box({ x: 5, y: 5 }))).toMatchObject({ left: 5, top: 5, width: 0, height: 0 });
  });
});
...
describe('withinGroup', () => {
  it('re-expresses a child box relative to the group’s centre', () => {
    ...
    expect(withinGroup(box({ x: 10, y: 10, width: 50, height: 50 }), { width: 200, height: 200 })).toMatchObject(
