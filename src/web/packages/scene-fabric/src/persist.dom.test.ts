...
import { setObjectAssetReference, VIGILIA_ASSET_PROPERTY } from './object-asset.js';
...
function keysOf(scene: { readonly objects: readonly Readonly<Record<string, unknown>>[] }, index = 0): string[] {
...
function image(): FabricImage {
  return new FabricImage(null as unknown as HTMLImageElement, { width: 10, height: 10 });
}
...
describe('the persisted key set, per class', () => {
  ...
  const CASES: readonly (readonly [string, () => object, readonly string[]])[] = [
    ['Rect', () => new Rect({ width: 10, height: 10 }), ['height', 'id', 'left', 'top', 'type', 'version', 'width']],
    [
      'Ellipse',
    ...
    [
      'FabricText',
      () => new FabricText('hi'),
      ['height', 'id', 'left', 'styles', 'text', 'top', 'type', 'version', 'width'],
    ],
    [
      'Textbox',
    ...
    [
      'FabricImage',
      image,
      ['crossOrigin', 'filters', 'height', 'id', 'left', 'src', 'top', 'type', 'version', 'width'],
    ],
    [
      'Group',
    ...
    [
      ...
      ['family', 'height', 'id', 'left', 'settings', 'top', 'type', 'version', 'width'],
    ],
  ];

  it.each(CASES)('%s persists exactly its authored surface', (_name, make, expected) => {
    const object = make();
...
describe('identity survives a round trip', () => {
  it('persists an image asset identity separately from its transient source URL', async () => {
    const source = image();
    source.set('id', 'logo-image');
    ...
    expect(revived.getObjects()[0]!.get(VIGILIA_ASSET_PROPERTY)).toEqual({ assetId: 'logo', kind: 'svg' });
  });
  ...
  it('carries an id on every object, nested ones included', async () => {
    ...
    expect(objects.map((object) => object.get('id'))).toEqual(['rect', 'group']);
    expect((objects[1] as Group).getObjects().map((object) => object.get('id'))).toEqual(['child']);
  });
  ...
  it('matches by id and never by position', async () => {
    ...
    const found = revived.getObjects().find((object) => object.get('id') === 'bound-chart');
...
describe('a settings object cannot be mutated in place', () => {
  it('freezes what it was given, deeply', () => {
    ...
    const chart = new VigiliaChart(chartOptions({ settings } as Partial<VigiliaChartOptions>));
    ...
    expect(() => {
      (chart.settings as unknown as Record<string, unknown>)['unit'] = 'changed';
    }).toThrow(TypeError);
  });
...
describe('there is exactly one owner of scene serialisation', () => {
  ...
  it('scans real sources', () => {
    ...
    expect(SCENE_PERSISTED_PROPERTIES).toEqual(['id', VIGILIA_TEXT_PROPERTY, VIGILIA_PAINT_PROPERTY, VIGILIA_ASSET_PROPERTY]);
  });
});
