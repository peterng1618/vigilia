...
export function applyObjectPalettePaints(canvas: StaticCanvas, globals: Globals | undefined): void {
  if (typeof (canvas as unknown as { getObjects?: unknown }).getObjects !== 'function') return;
  applyPaints(canvas.getObjects(), globals);
  canvas.requestRenderAll();
}

function applyPaints(objects: readonly PaintableObject[], globals: Globals | undefined): void {
  for (const object of objects) {
    ...
    if (isPaintRefs(refs)) {
      for (const [property, ref] of Object.entries(refs)) {
        ...
        const paint = fabricArtboardPaint(value, extent(object.width, object.scaleX), extent(object.height, object.scaleY));
...
function isPaintRefs(value: unknown): value is FabricPaintRefs {
  ...
  return Object.entries(value).every(([property, ref]) =>
    (property === 'fill' || property === 'stroke') && typeof ref === 'string' && ref.startsWith('palette.'),
  );
}
