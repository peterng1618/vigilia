...
describe('background media', () => {
  it('mounts fitted image media below the Fabric canvas and disposes its source', () => {
    ...
    const handle = mountBackgroundMedia({
      host,
      artboard: { width: 400, height: 200, backgroundMedia: { assetId: 'hero', fit: 'contain' } },
      assets: [{ id: 'hero', kind: 'image', path: 'assets/hero.png' }],
      resolveAsset: () => ({ url: 'blob:hero', dispose: () => { disposed = true; } }),
    });

    const image = host.querySelector<HTMLImageElement>('[data-vigilia-background-media] img');
    expect(image?.src).toBe('blob:hero');
    expect(image?.style.objectFit).toBe('contain');
    expect(host.firstElementChild?.hasAttribute('data-vigilia-background-media')).toBe(true);
  ...
  it('mounts muted looping video media', () => {
    ...
    const handle = mountBackgroundMedia({
      host,
      artboard: { width: 400, height: 200, backgroundMedia: { assetId: 'loop', fit: 'cover' } },
      assets: [{ id: 'loop', kind: 'video', path: 'assets/loop.webm' }],
      resolveAsset: () => ({ url: 'blob:loop' }),
    });

    const video = host.querySelector<HTMLVideoElement>('[data-vigilia-background-media] video');
  ...
  it('replaces the old source when its media changes', () => {
    ...
    const handle = mountBackgroundMedia({
      host,
      artboard: { width: 400, height: 200, backgroundMedia: { assetId: 'one', fit: 'cover' } },
      assets: [{ id: 'one', kind: 'image', path: 'assets/one.png' }],
      resolveAsset: (id) => ({ url: `blob:${id}`, dispose: () => { disposed.push(id); } }),
    });

    handle.update({
      artboard: { width: 400, height: 200, backgroundMedia: { assetId: 'two', fit: 'contain' } },
