...
function createValidPackage(id = 'living-room', name = 'Living Room'): Uint8Array {
...
): Promise<{ status: number; headers: Record<string, string | string[] | undefined>; body: Buffer; json: () => unknown; text: () => string }> {
  return new Promise((resolve) => {
    const req = new Readable({
      read() {
        if (body !== undefined) {
          this.push(typeof body === 'string' ? Buffer.from(body) : Buffer.from(body));
        }
        this.push(null);
      },
    ...
    if (body !== undefined) {
      req.headers['content-length'] = String(typeof body === 'string' ? Buffer.byteLength(body) : body.byteLength);
    }
    ...
    res.writeHead = ((status: number, headers?: http.OutgoingHttpHeaders | http.OutgoingHttpHeader[]) => {
      statusCode = status;
...
describe('Host theme routes', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'vigilia-host-theme-test-'));
  ...
  it('handles theme save, list, document fetch, and package fetch', async () => {
    const putRes = await request(hosted.server, 'PUT', '/api/themes/living-room', validEmptyAssetPackage);
    expect(putRes.status).toBe(200);

    const listRes = await request(hosted.server, 'GET', '/api/themes');
    expect(listRes.status).toBe(200);
    const listData = listRes.json() as { themes: readonly { id: string; name: string }[] };
    expect(listData.themes).toEqual([{ id: 'living-room', name: 'Living Room', updatedAt: expect.any(String) }]);

    const docRes = await request(hosted.server, 'GET', '/api/themes/living-room/document');
  ...
  it('forbids PUT from non-loopback addresses (§7)', async () => {
    const res = await request(hosted.server, 'PUT', '/api/themes/living-room', validEmptyAssetPackage, {
  ...
  it('allows display GET reads from LAN addresses', async () => {
    ...
    const docRes = await request(hosted.server, 'GET', '/api/themes/living-room/document', undefined, {
      remoteAddress: '192.168.1.50',
    });
    expect(docRes.status).toBe(200);

    const listRes = await request(hosted.server, 'GET', '/api/themes', undefined, {
      remoteAddress: '192.168.1.50',
    });
    expect(listRes.status).toBe(200);
  });
  ...
  it('rejects malformed PUT packages and preserves existing package', async () => {
    ...
    const badRes = await request(hosted.server, 'PUT', '/api/themes/living-room', new Uint8Array([0, 1, 2, 3]));
    expect(badRes.status).toBe(400);

    const docRes = await request(hosted.server, 'GET', '/api/themes/living-room/document');
    expect(docRes.status).toBe(200);
    expect((docRes.json() as FabricThemeEnvelope).metadata?.name).toBe('Living Room');
  });

  it('rejects invalid or traversal theme IDs', async () => {
    const putRes = await request(hosted.server, 'PUT', '/api/themes/..%2Fescape', validEmptyAssetPackage);
    expect(putRes.status).toBe(400);

    const getRes = await request(hosted.server, 'GET', '/api/themes/..%2Fescape/document');
