...
describe('resolveStaticPath', () => {
  ...
  it('ignores a query string', () => {
    expect(resolveStaticPath(ROOT, '/main.js?v=2')).toBe(path.join(ROOT, 'main.js'));
  });
  ...
  describe('refusing to escape the root (§141)', () => {
    ...
    it('refuses a sibling directory that merely shares the root prefix', () => {
      ...
      expect(resolveStaticPath(ROOT, '/../player-secrets/key.pem')).toBeUndefined();
    });
    ...
    it('checks where the path LANDS, not whether the URL looked suspicious', () => {
      ...
      expect(resolveStaticPath(ROOT, '/assets/../main.js')).toBe(path.join(ROOT, 'main.js'));
    });
  });
