...
export const OS_DESCRIPTORS: readonly SensorDescriptor[] = OS_KEYS.map(
  (key) => {
      ...
      sample:
        load === undefined
          ? missing(
              sensorId,
              timestamp,
              "waiting for a second reading to measure load against",
            )
    ...
    entries.push({
      ...
      sample:
        next.totalMemBytes > 0
          ? ok(
              sensorId,
              (usedBytes / next.totalMemBytes) * 100,
              unitFor("ram.used.percent"),
              timestamp,
            )
