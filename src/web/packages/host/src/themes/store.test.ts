...
function createValidEmptyPackage(id = 'living-room', name = 'Living Room'): Uint8Array {
...
describe('ThemeStore', () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'vigilia-theme-store-test-'));
  });
  ...
  it('lists themes with metadata only', async () => {
    const store = createThemeStore(tmpDir);
    await store.write('theme-a', createValidEmptyPackage('theme-a', 'Theme Alpha'));
    await store.write('theme-b', createValidEmptyPackage('theme-b', 'Theme Beta'));
    ...
    expect((list[0] as unknown as { envelope?: unknown }).envelope).toBeUndefined();
  });
  ...
  it('refuses invalid or path traversal theme IDs', async () => {
    ...
    await expect(store.write('../escape', validBytes)).rejects.toThrow('Invalid theme id');
    await expect(store.write('/etc/passwd', validBytes)).rejects.toThrow('Invalid theme id');
    await expect(store.write('', validBytes)).rejects.toThrow('Invalid theme id');
    await expect(store.write('bad id with spaces', validBytes)).rejects.toThrow('Invalid theme id');
