...
describe('ThemeLibraryClient', () => {
  it('saves and opens packages through the host theme routes', async () => {
    ...
    const client = createThemeLibraryClient({ fetch: mockFetch as unknown as typeof fetch });
  ...
  it('lists themes with metadata', async () => {
    const mockFetch = vi.fn(async () => {
      return new Response(
        JSON.stringify({
          themes: [
            { id: 'living-room', name: 'Living Room', updatedAt: '2026-09-19T00:00:00.000Z' },
          ],
        }),
        { status: 200 },
      );
  ...
  it('throws descriptive error on save failure', async () => {
    const mockFetch = vi.fn(async () => new Response('Invalid package', { status: 400 }));
    const client = createThemeLibraryClient({ fetch: mockFetch as unknown as typeof fetch });

    await expect(client.save('living-room', new Uint8Array([1]))).rejects.toThrow(
      'Could not save theme "living-room" (400)',
    );
  });
  ...
  it('refuses invalid theme IDs without fetching', async () => {
    ...
    await expect(client.save('../escape', new Uint8Array())).rejects.toThrow('Invalid theme id');
