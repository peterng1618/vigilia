...
describe('font preview', () => {
  it('loads a transient face and releases only that face', async () => {
    ...
    const handle = await previewFontFace(face, {
      fetch: vi.fn().mockResolvedValue(new Response(new Uint8Array([1, 2, 3]), { status: 200 })),
  ...
  it('replaces an in-flight preview without retaining the old face', async () => {
    ...
    const fonts = { add: vi.fn(), delete: vi.fn() };
    const createFontFace = vi.fn(() => ({ load: vi.fn().mockResolvedValue(undefined) }));
    const first = previewFontFace(face, { fetch: vi.fn(() => firstFetch), createFontFace, fonts });
    const second = await previewFontFace(trio.faces[1]!, {
      fetch: vi.fn().mockResolvedValue(new Response(new Uint8Array([4]), { status: 200 })), createFontFace, fonts,
    });
  ...
  it('does not retain a failed fetch or FontFace load', async () => {
    ...
    await expect(previewFontFace(face, {
      fetch: vi.fn().mockResolvedValue(new Response(null, { status: 404 })),
      createFontFace: vi.fn(), fonts,
    ...
    await expect(previewFontFace(face, {
      ...
      createFontFace: vi.fn(() => ({ load: vi.fn().mockRejectedValue(new Error('invalid font')) })), fonts,
