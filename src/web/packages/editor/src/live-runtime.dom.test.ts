  ...
  it('replaces a bound text value from its runtime source without changing authored runs', () => {
    ...
    const authored = {
      runs: [
        ...
        { kind: 'value' as const, bindingId: 'load', precision: 0, unitDisplay: 'short' as const },
      ],
    };
    ...
    runtime.setSource({
      latest: (key) => key === 'cpu.load'
        ? { sensorId: 'cpu.load', timestamp: '2026-09-20T00:00:00.000Z', status: 'ok', value: 48, unit: '%' }
        : undefined,
      history: () => [],
    });
