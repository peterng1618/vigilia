...
describe("the adopted editor shell", () => {
  it("installs the disposal-aware history hook at the fork boundary", async () => {
    const editor = {
      canvas: {
        toObject: vi.fn(() => ({ version: "7.4.0", objects: [] })),
        setDimensions: vi.fn(),
    ...
    expect((host.firstElementChild as HTMLElement).style.cssText).toContain(
      "width: 800px",
    );
    expect((host.firstElementChild as HTMLElement).style.cssText).toContain(
      "height: 450px",
    ...
    expect(editor.canvas.setDimensions).toHaveBeenCalledWith({
      width: 800,
      height: 450,
    });
    expect(editor.canvas.setViewportTransform).toHaveBeenCalledWith([
      0.625, 0, 0, 0.625, 0, 0,
    ]);
    expect(
      shell.snapshot({ id: "theme", artboard: { width: 1280, height: 720 } }),
    ).toMatchObject({ schemaVersion: 2, id: "theme" });

    shell.setFitMode("cover");
    expect(editor.canvas.setDimensions).toHaveBeenLastCalledWith({
      width: 1066.6666666666667,
      height: 600,
    });
    expect(editor.canvas.setViewportTransform).toHaveBeenLastCalledWith([
      0.8333333333333334, 0, 0, 0.8333333333333334, 0, 0,
    ]);
  ...
  it("reconciles a supplied shared scene onto the fork canvas", async () => {
    ...
    const adapter = vi
      .spyOn(sceneFabric, "createSceneAdapter")
      .mockReturnValue({
        apply,
        dispose: vi.fn(),
        objectFor: vi.fn(),
  ...
  it("validates and revives a supplied Fabric envelope before extensions adopt it", async () => {
    const editor = {
      ...
      historyManager: { resetHistory: vi.fn() },
      destroy: vi.fn(),
    };
    initEditor.mockResolvedValue(editor);
    const sceneFabric = await import("@vigilia/scene-fabric");
    const revive = vi
      .spyOn(sceneFabric, "reviveThemeEnvelope")
      .mockResolvedValue();
    const adapter = vi
      .spyOn(sceneFabric, "createSceneAdapter")
      .mockReturnValue({
    ...
    const envelope = {
      ...
      globals: {
        palette: {
          ...
          background: {
            name: "Background",
            value: { kind: "solid", color: "#101216" },
          },
          bars: { name: "Bars", value: { kind: "solid", color: "#000000" } },
        },
      },
  ...
  it("preserves the active stage when initialization fails", async () => {
    ...
    await expect(
      mountForkShell({ host, artboard: { width: 1, height: 1 } }),
