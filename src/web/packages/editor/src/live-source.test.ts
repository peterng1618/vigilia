...
describe('createEditorSource', () => {
  ...
  it('reports a live disconnect without inventing samples', () => {
    ...
    const live = createEditorSource({ mode: 'live', keys: ['cpu.load'], onStatus });
    ...
    const now = Date.now();
    const event = JSON.stringify(createBatch([{ semanticKey: 'cpu.load', sample: {
      sensorId: 'cpu.load', timestamp: new Date(now).toISOString(), status: 'ok', value: 42, unit: '%',
    } }], now));
