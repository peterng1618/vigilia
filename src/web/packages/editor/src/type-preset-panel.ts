...
export function createTypePresetPanel(host: HTMLElement, onChange: (presets: TypePresets) => void): TypePresetPanel {
  ...
  const controls = (entry: { readonly name: string; readonly value: TypePreset }): HTMLElement[] => {
    ...
    return [label('Name', name), label('Family', family), label('Size', size), label('Weight', weight), label('Line height', lineHeight)];
  };
