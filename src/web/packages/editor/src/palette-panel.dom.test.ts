...
describe('palette panel', () => {
  const palette = {
    ...
    background: { name: 'Background', value: { kind: 'solid' as const, color: '#102030' } },
  };
  ...
  it('edits a Fabric-compatible solid colour without changing its stable id', () => {
    ...
    const color = document.querySelector<HTMLInputElement>('[data-vigilia-palette-color]')!;
    color.value = 'rgb(16 32 48)';
    color.dispatchEvent(new Event('change'));

    expect(change).toHaveBeenLastCalledWith({
      ...palette,
      background: { name: 'Background', value: { kind: 'solid', color: 'rgb(16 32 48)' } },
    });
  });
  ...
  it('creates gradients with editable validated stops', () => {
    ...
    token.value = 'background';
    token.dispatchEvent(new Event('change'));
    const kind = document.querySelector<HTMLSelectElement>('[data-vigilia-palette-kind]')!;
    kind.value = 'gradient';
    kind.dispatchEvent(new Event('change'));
    const angle = document.querySelector<HTMLInputElement>('[data-vigilia-palette-angle]')!;
    angle.value = '45';
    ...
    document.querySelector<HTMLButtonElement>('[data-vigilia-palette-add-stop]')!.click();
    expect(change).toHaveBeenLastCalledWith(expect.objectContaining({
  ...
  it('keeps the reserved transparent token immutable', () => {
    ...
    expect(document.querySelector<HTMLInputElement>('[data-vigilia-palette-name]')!.disabled).toBe(true);
