...
import { VIGILIA_PAINT_PROPERTY, VIGILIA_TEXT_PROPERTY } from '@vigilia/scene-fabric';
import { createNewPaintDefaults, createNewTextDefaults } from './new-object-defaults.js';
...
const globals = {
  ...
  typePresets: {
    body: { name: 'Body', value: { family: 'Inter', size: 16, weight: 500, lineHeight: 1.2 } },
  },
};
...
describe('new object defaults', () => {
  ...
  it('derives per-run palette and type-preset references for new text', () => {
    ...
    expect(defaults).toMatchObject({
      fill: '#102030', fontFamily: 'Inter', fontSize: 16, fontWeight: 500, lineHeight: 1.2,
      [VIGILIA_PAINT_PROPERTY]: { fill: 'palette.ink' },
      [VIGILIA_TEXT_PROPERTY]: { runs: [{ text: 'New text', typePreset: 'typePresets.body', style: { color: { ref: 'palette.ink' } } }] },
    });
  });
  ...
  it('refuses to invent local paint or type values when references are unavailable', () => {
    ...
    expect(() => createNewTextDefaults({ palette: globals.palette }, 'New text')).toThrow('type preset');
  });
});
