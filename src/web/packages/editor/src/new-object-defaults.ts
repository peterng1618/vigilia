...
export interface NewTextDefaults extends NewPaintDefaults {
  ...
  readonly [VIGILIA_TEXT_PROPERTY]: {
    readonly runs: readonly [{
      ...
      readonly style: { readonly color: { readonly ref: `palette.${string}` } };
    }];
  };
}

/** Supplies valid authored references without making defaults document state. */
export function createNewPaintDefaults(globals: FabricGlobals | undefined): NewPaintDefaults {
  const [id, entry] = firstPalette(globals);
  const ref = `palette.${id}` as const;
  const fill = fabricArtboardPaint(entry.value, 1, 1);

  if (fill === undefined) throw new Error(`Palette token "${ref}" cannot paint a new object.`);

  return { fill, [VIGILIA_PAINT_PROPERTY]: { fill: ref } };
}

/** Supplies the paint and per-run typography required by a new fork text object. */
export function createNewTextDefaults(globals: FabricGlobals | undefined, text: string): NewTextDefaults {
  ...
  if (color === undefined) throw new Error('New text requires a palette reference.');
  ...
  return {
    ...
    [VIGILIA_TEXT_PROPERTY]: {
      runs: [{ kind: 'literal', text, typePreset, style: { color: { ref: color } } }],
    },
  };
}

function firstPalette(globals: FabricGlobals | undefined): readonly [string, NonNullable<FabricGlobals['palette']>[string]] {
  const entries = Object.entries(globals?.palette ?? {});
  const selected = entries.find(([id]) => id !== 'none') ?? entries.find(([id]) => id === 'none');
  if (selected === undefined) throw new Error('New objects require a palette token.');
  return selected;
}

function firstTypePreset(globals: FabricGlobals | undefined): readonly [string, TypePreset] {
