...
describe('new object panel', () => {
  it('delegates text construction to the fork with derived v2 defaults', () => {
    const addText = vi.fn();
    const root = createNewObjectPanel(document.body, { textManager: { addText } } as never, {
      palette: {
    ...
    expect(addText).toHaveBeenCalledWith(expect.objectContaining({
      text: 'New text', fill: '#102030',
      [VIGILIA_PAINT_PROPERTY]: { fill: 'palette.ink' },
      [VIGILIA_TEXT_PROPERTY]: expect.objectContaining({
        runs: [expect.objectContaining({ text: 'New text', typePreset: 'typePresets.body', style: { color: { ref: 'palette.ink' } } })],
      }),
    }));
  });
});
