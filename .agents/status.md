...
| Check | Result |
|---|---|
| Settings scope — this PC, and a theme's own questions | A machine-level choice was being filtered by the active theme, so choosing a theme that reads no disks hid the drive the consumer had set. The page now keeps this PC's settings complete whatever theme is shown, derives each theme's questions from its own bindings (`required-devices.ts`), asks them once and stores the answer per theme (`theme-answers.json` beside the global `devices.json`), resolving through one path: theme answer → global answer → provider default. Driving the page against the real host found three defects that reading it had not: saving a device choice, choosing a theme and answering a theme question each changed what the providers read without publishing it, so a display kept the device the consumer had just replaced (fixed at the one boundary, `publishAssignment()`); and the questions were rendered before the devices they offer were read, so the first time a theme raised a question its picker offered only "Use this PC's settings" (fixed in the page's load path). Each fix was re-run disabled and the new tests fail without it. Verified against the real host: the device legends are identical with no theme chosen, a theme reading nothing and a theme reading a disk; the disk theme asks exactly one question, and once answered the section stays hidden across a reload while its stored answer overrides a different global choice and the global section keeps showing that choice. Evidence: 1222-unit suite, `biome check` exit 0, seven-project typecheck, builds, 283.8 KB size gate, full browser suite 97 passed / 47 skipped / 0 failed, and an inspected `settings-theme-question` capture. |
| Clock and date as sensors | The starter theme's clock was authored literal text (`"07:24"`), so it looked like a clock and showed one time for ever. `time.now` and `date.today` now come from a host provider as **instants**: the reading carries the offset of the zone it was read in, so two displays agree and none of them re-converts. An author owns how it reads, with a small token set (`HH H hh h mm ss dddd ddd DD D MMMM MMM MM YYYY YY A a`, `[bracketed]` literals, unknown tokens kept literally rather than blanked) and **which zone each clock shows**: the Design tab's run row gained a **Reads** picker, a **Format** field with a live preview and a **Zone** picker whose first option follows the display and whose list leads with UTC. The consumer owns only the default zone, as a global setting (`/settings` → Display, stored in `display.json` beside the themes) applied by the provider. Two defects were found by driving it rather than reading it: an unbind left a stale `run.bindingId`, and the e2e probe's `adapter.objectFor` indexes only what a plan applied — so on a scene revived from a package every object read as missing, which is what the new clock test tripped over. Verified against the real host: the painted clock advanced on its own across cadences (17:39:08 → 17:39:10), and after `PUT /api/display {"timeZone":"Asia/Tokyo"}` the painted reading matched Tokyo's wall clock while an unchosen zone matched this PC's. Verified in the editor: pinning `Asia/Tokyo` on the starter clock left the rebuilt control reading Tokyo, with the preview matching that zone's wall clock rather than this PC's — and the zone tests were re-run with the write disabled, where both fail. Evidence: 1217-unit suite, `biome check` exit 0, seven-project typecheck, builds, 283.8 KB size gate, full browser suite 93 passed / 41 skipped / 0 failed, and an inspected `editor-fork-text-reads` capture showing Reads / Format / preview / Zone. |
| Theme thumbnails | The library showed themes as text, so choosing between them meant opening them. A still of the artboard is now captured in the editor at save time — from the canvas already on screen with its fonts loaded, so no second renderer and no new host dependency — stored beside the package rather than inside it (the package format admits only manifest.json, theme.json and assets/, and a thumbnail is a rendering of one machine's fonts). `GET /api/themes/:id/thumbnail` serves it and a missing one is a 404 the library falls back from, so older packages still list; `PUT` is loopback-only, bounded to 2 MiB and PNG-checked; a capture failure warns and never fails the save. Verified end to end: saving stored a 75 KB PNG, the route served it as image/png, and the library page decoded it at 626x352 (the artboard's aspect), with an inspected capture showing the rendered dashboard beside the theme name. Evidence: 1161-unit suite, `biome check` exit 0, seven-project typecheck, builds, 281.9 KB size gate, browser suite 91 passed / 0 failed. || Author journey — selection inspector and text | An author can now change what they selected. The Design tab carries X/Y/width/height/rotation, opacity with the object's paint reference **and what it resolves to** ("palette.panel -> #081523d9"), run rows with per-run preset and colour, and text alignment/wrap/overflow. Text is editable in place on double-click, which Fabric does not wire itself: its own double-click handler only selects a word while already editing. Value runs read as their token while authoring (`@cpu.load`), with unmapped and undeclared bindings shown distinctly, and the values remain one View-menu choice away. Five defects were found by driving it rather than reading it: new text was created half off the artboard and painted in the artboard colour (invisible, therefore unselectable); a run's colour never reached Fabric because `refreshBoundText` only handles objects a sample resolves; single runs were skipped outright; `applyAuthoredText` never reapplied alignment, so every layout change was invisible; and new text used the smallest preset (an 11px caption) instead of a body size. Verified in a browser with inspected captures. Evidence: 1155-unit suite, `biome check` exit 0, seven-project typecheck, builds, 281.9 KB size gate, browser suite 91 passed / 0 failed. |
| Consumer journey — choose, configure, forget | A consumer may never open the editor, so they must be able to take a theme and answer only what it needs. `/settings` now covers both decisions under one heading: Themes lists every saved theme by name and author with the chosen one marked "Showing", and Devices asks which hardware it reports. The host remembers the chosen theme beside the device assignments and `/` resolves through it (explicit link, then the choice, then the only theme, then the library, then a first-run page); the choice survives a restart, and a stale one falls back rather than breaking. Verified against the real host: two themes listed and distinguishable by id, choosing one marked and persisted it, and the devices section rendered real hardware. || Two-disk slot and provider parity | A theme showing a system disk and a data disk had no way to express the second: the data-disk assignment was stored and offered in the UI but never read, so choosing it did nothing. The vocabulary now has `disk.data.*` as a second slot, and both providers answer it from the assigned volume; the library provider also scopes the aggregate disk keys to an assigned system disk, matching what the same assignment means for LHM. The library lists volumes by mount (what it can resolve) rather than model. Verified 2026-09-24 without LHM on this machine: C: as system and D: as data gave `disk.total` 465 GB and `disk.data.total` 3726 GB with 61.2% used, where an unassigned host sums every volume. A missing `disk.data.used.percent` in the library slot was also found and fixed. Evidence: 1132-unit suite, `biome check` exit 0, seven-project typecheck, builds, 281.8 KB size gate, browser suite 91 passed / 0 failed. |
...
- `/api/health` reports requested-but-unanswered keys under `unmapped`.
  ...
  appear there.
- The clock provider answers `time.now`/`date.today` from this PC's clock,
  read in the zone `/settings` stores (`display.json`); it needs no platform
  API, so a thin host still has a clock.
...
- The browser suite starts the real host and covers hosted theme loading,
  package-asset serving, the live SSE batch count, the painted clock advancing
  with the chosen zone and the loopback admin guard
...
1. Device assignment has an API and a page, but the editor does not offer a
   device picker while authoring, and a theme cannot declare which group a chart
   wants.
...
- An answered theme question cannot be changed from `/settings`: the section
  hides once the theme has an answer (spec'd as "asked once, then not again"),
  so changing one means editing `theme-answers.json`. Re-showing answered
  questions with their current value is a product-taste call, not made here.
