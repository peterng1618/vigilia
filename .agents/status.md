...
| LHM packaging and launch | `npm run vendor:lhm` stages the pinned v0.9.6 release unpacked into `packages/host/vendor/lhm`, adds the `LICENSE` and `THIRD-PARTY-NOTICES.txt` the archive omits, and records the sha256 in `PROVENANCE.txt`. Bundling approved 2026-09-24. Launching is opt-in (`--lhm-exe`) because LHM's manifest is `requireAdministrator` — it loads a kernel driver. `spawn` cannot elevate, so the launcher now runs LHM's own startup task when it exists (already approved, so it prompts for nothing) and otherwise requests elevation through Windows' runas verb; `--lhm-exe` previously could not work at all. A failed launch reports "was asked to start" rather than claiming success. Verified 2026-09-24 against the real binary: elevation detected, a missing file and a source file correctly not, the registered-task check working, and a plain binary still spawned directly. **Not verified:** approving the consent prompt and registering the elevated task both need administrator rights this session lacks. Evidence: 1122-unit suite, `biome check` exit 0, seven-project typecheck, builds, 281.6 KB size gate, browser suite 91 passed / 0 failed. |
