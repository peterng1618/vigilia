...
| Check | Result |
|---|---|
| Clock and date as sensors | The starter theme's clock was authored literal text (`"07:24"`), so it looked like a clock and showed one time for ever. `time.now` and `date.today` now come from a host provider as **instants**: the reading carries the offset of the zone it was read in, so two displays agree and none of them re-converts. An author owns how it reads, with a small token set (`HH H hh h mm ss dddd ddd DD D MMMM MMM MM YYYY YY A a`, `[bracketed]` literals, unknown tokens kept literally rather than blanked); the Design tab's run row gained a **Reads** picker and a **Format** field with a live preview, so a binding can finally be authored rather than only pre-built. The consumer owns only the zone, as a global setting (`/settings` → Display, stored in `display.json` beside the themes) applied by the provider. Two defects were found by driving it rather than reading it: an unbind left a stale `run.bindingId`, and the e2e probe's `adapter.objectFor` indexes only what a plan applied — so on a scene revived from a package every object read as missing, which is what the new clock test tripped over. Verified against the real host: the painted clock advanced on its own across cadences (17:39:08 → 17:39:10), and after `PUT /api/display {"timeZone":"Asia/Tokyo"}` the painted reading matched Tokyo's wall clock while an unchosen zone matched this PC's. Evidence: 1215-unit suite, `biome check` exit 0, seven-project typecheck, builds, 283.8 KB size gate, full browser suite 92 passed / 40 skipped / 0 failed, and an inspected `editor-fork-text-reads` capture. |
...
- An author cannot yet pin one clock to a named zone in the editor:
  `Binding.timeZone` is validated, honoured by the formatter and unit-tested,
  but no control writes it, so a world-clocks theme is authored in the envelope
